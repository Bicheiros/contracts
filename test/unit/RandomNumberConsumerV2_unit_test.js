...
!developmentChains.includes(network.name)
  ...
  : describe("RandomNumberConsumer Unit Tests", async function () {
      ...
      it("dataFetched changes the value in accord to setDataFetched", async () => {

        let boolResult = await randomNumberConsumerV2.dataFetched()
        assert(boolResult == false)

        let setData = await randomNumberConsumerV2.setDataFetched(true)
        await setData.wait()

        boolResult = await randomNumberConsumerV2.dataFetched()
